...
def main():
    ...
    for idx in tqdm(indices, desc=f"GPU{args.gpu_id}"):
        try:
            ...
            # Output directory: per-video subdirectory under lipsync_features/
            # CRITICAL: must be unique per video — using the shared video dir causes
            # all videos to overwrite the same lipsync_f_s.npy / lipsync_A.npy files.
            video_stem   = os.path.splitext(os.path.basename(video_path))[0]
            lipsync_root = os.path.join(os.path.dirname(os.path.dirname(video_path)),
                                        "lipsync_features")
            output_dir   = os.path.join(lipsync_root, video_stem)
